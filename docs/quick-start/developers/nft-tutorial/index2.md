...
state variables, user interactions, muti-token contract, nft-creation and nft transfer to eventually create a basic NFT
...
mkdir nft
cd nft
dotnet new aelf -n NFT
```

### Adding Your Smart Contract Code

Now that we have a template NFT project, we can customise the template to incorporate our own contract logic.
...
- Rename the file name from `Protobuf/contract/hello_world_contract.proto` to `nft_contract.proto`:

```bash title="Terminal"
mv Protobuf/contract/hello_world_contract.proto Protobuf/contract/nft_contract.proto
```

- open the project with your IDE.

The implementation of file `src/Protobuf/contract/nft_contract.proto` is as follows:

```csharp title="nft_contract.proto"
syntax = "proto3";
...
service NFT {
  ...
  option (aelf.base) = "Protobuf/reference/acs12.proto";
    ...
    rpc GetNFTOwner(google.protobuf.Int64Value) returns (aelf.Address) {
        option (aelf.is_view) = true;
    }
}
...
The implementation of file `src/NFTState.cs` is as follows:
...
namespace AElf.Contracts.NFT
{
    ...
    public class NFT : NFTContainer.NFTBase
    {
       ...
       public override Empty Initialize(Empty input)
        ...
        public override Address GetNFTOwner(Int64Value input)
        {
            var nft = State.NFTTokens[input.Value];
            Assert(nft != null, "Token does not exist.");
            return nft.Owner;
...
You should see **NFT.dll.patched** in the directory `nft/src/bin/Debug/net.6.0`
